...
This talk is delivered in both regional sessions:
...
Same hands-on report, two time zones.
...
[Read the post on LinkedIn](https://www.linkedin.com/posts/peng-zhang-4522a0127_finally-i-have-configured-claude-code-on-activity-7433203325115719680-gj3U){.smaller}
:::
::: {.column width="45%"}
![](image/image.png){fig-alt="LinkedIn post describing a six-month AI exploration roadmap"}
:::
:::
...
::: {.columns}
::: {.column width="50%"}
![](image/image-1.png){fig-alt="Claude Code conversational session designing a group sequential trial"}

*Conversational design — it asks for inputs one at a time.*
:::
::: {.column width="50%"}
![](image/image-2.png){fig-alt="Generated design assumptions and interim/final analysis plan tables"}
...
![](image/image-4.png){fig-alt="Clinical AI Agent Shiny app showing a GLP-1 receptor agonist competitive landscape" width="78%" fig-align="center"}
...
- Installed **Claude Code** inside RStudio — yes, through its **terminal**
- Then had Claude Code:
  - read **90K+ lines** of source data
  - generate an **HTML visualization**
  - **split the data by domain** to get a sense of it
  - reference the **SDTM IG** via an index (saves context window)
  - write the **SDTM spec** as **.yaml** (human *and* machine readable), per domain
  - write the **R code** from the .yaml
  - generate the **SDTM datasets** + check logs; resolve all coding errors
  - do the same for **ADaM**
:::
::: {.column width="42%"}
![](image/image-3.png){fig-alt="RStudio with Claude Code running in the terminal on the pilot7 project"}
